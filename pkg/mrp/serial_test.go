...
func TestSerialComparator_IsSerialInRange(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			targetSerial: "SN025",
			effectivity:  SerialEffectivity{FromSerial: "SN001", ToSerial: "SN050"},
			expected:     true,
		},
		{
		...
		},
		{
			...
			targetSerial: "SN001",
			effectivity:  SerialEffectivity{FromSerial: "SN010", ToSerial: "SN050"},
			expected:     false,
		},
		{
			name:         "in_open_range",
			targetSerial: "SN100",
			effectivity:  SerialEffectivity{FromSerial: "SN051", ToSerial: ""},
			expected:     true,
		},
		{
...
func TestSerialComparator_ResolveSerialEffectivity(t *testing.T) {
	...
	tests := []struct {
		name         string
		targetSerial string
		expectedParts []PartNumber
	}{
		{
			name:         "early_serial_gets_v1",
			targetSerial: "SN020",
			expectedParts: []PartNumber{"TURBOPUMP_V1", "COMMON_PART"},
		},
		{
			name:         "late_serial_gets_v2",
			targetSerial: "SN030",
			expectedParts: []PartNumber{"TURBOPUMP_V2", "COMMON_PART"},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i, expectedPart := range tt.expectedParts {
				if result[i].ChildPN != expectedPart {
	...
	tests := []struct {
		...
		},
		{
			...
			bomLines: []BOMLine{
				...
				{
					ParentPN: "ENGINE", ChildPN: "PART_A",
					Effectivity: SerialEffectivity{FromSerial: "SN040", ToSerial: "SN060"},
				},
			},
			expectErr: true,
		},
