...
func TestItem_Validation(t *testing.T) {
	...
	testCases := []struct {
		...
		{"zero lead time", "PART", "desc", 0, LotForLot, 0, 0, "EA", "lead time must be positive, got 0"},
		{"negative lead time", "PART", "desc", -1, LotForLot, 0, 0, "EA", "lead time must be positive, got -1"},
		{"negative min order qty", "PART", "desc", 1, LotForLot, -1, 0, "EA", "minimum order quantity cannot be negative, got -1"},
		{"negative safety stock", "PART", "desc", 1, LotForLot, 0, -1, "EA", "safety stock cannot be negative, got -1"},
		{"empty UOM", "PART", "desc", 1, LotForLot, 0, 0, "", "unit of measure cannot be empty"},
		{"MinimumQty with zero order qty", "PART", "desc", 1, MinimumQty, 0, 0, "EA", "lot sizing rule MinimumQty requires non-zero minimum order quantity"},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := NewItem(tc.partNumber, tc.description, tc.leadTime, tc.lotRule, tc.minOrderQty, tc.safetyStock, tc.uom)
			if err == nil {
