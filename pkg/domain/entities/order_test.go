...
func TestPlannedOrder_Validation(t *testing.T) {
	...
	validOrder, err := NewPlannedOrder("PART123", 5, startDate, dueDate, "test", "FACTORY", Make, "SN001")
	if err != nil {
	...
	testCases := []struct {
		...
		{"zero quantity", "PART", 0, startDate, dueDate, "FACTORY", "quantity must be positive, got 0"},
		{"negative quantity", "PART", -1, startDate, dueDate, "FACTORY", "quantity must be positive, got -1"},
		{"start after due", "PART", 5, dueDate, startDate, "FACTORY", "start date 2025-01-10 00:00:00 +0000 UTC cannot be after due date 2025-01-01 00:00:00 +0000 UTC"},
		{"empty location", "PART", 5, startDate, dueDate, "", "location cannot be empty"},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := NewPlannedOrder(tc.partNumber, tc.quantity, tc.startDate, tc.dueDate, "test", tc.location, Make, "SN001")
			if err == nil {
